...
def run(erfa_b=2, erfa_c=1):
    ...
    for i in range(len(xt) - 1):
        ...
        #             print(f"hold 黄金,美->比")
            ...
            result[2] = 0
        #             print(f"hold 黄金，比->美")
        elif xt[i + 1] * (1 - beta_c * 0.01) <= xt_1[i + 1] <= (1 + beta_c * 0.01) * xt[i + 1] and yt[i + 1] * (
                1 - beta_b * 0.01) <= yt_1[i + 1] <= (1 + beta_b * 0.01) * yt[i + 1]:
            result[1] = result[1] * (1 + (xt[i + 1] - xt[i]) / xt[i])
            result[2] = result[2] * (1 + (yt[i + 1] - yt[i]) / yt[i])
        #             print(f"全hold:{result}")

        elif xt_1[i + 1] > (1 + beta_c * 0.01) * xt[i + 1] and (xt_1[i + 1] - xt[i + 1]) * yt[i + 1] >= (
                yt_1[i + 1] - yt[i + 1]) * xt[i + 1]:
            result[1] = result[1] * (1 + (xt[i + 1] - xt[i]) / xt[i]) + \
            ...
            result[2] = 0
        #             print(f"全黄金:{result}")
        elif yt_1[i + 1] > (1 + beta_b * 0.01) * yt[i + 1] and (yt_1[i + 1] - yt[i + 1]) * xt[i + 1] >= (
                xt_1[i + 1] - xt[i + 1]) * yt[i + 1]:
            result[2] = result[2] * (1 + (yt[i + 1] - yt[i]) / yt[i]) + \
                        result[0] * (1 - erfa_b * 0.01) + \
...
def main():
    temp = [(95, 28),(49, 11),(13, 93),(17, 5)]  # 收益金额四分位点的 beta_b beta_c
    for k in range(4):
        global beta_b
        global beta_c
        ...
        for i in np.arange(1,11,0.1):
            for j in np.arange(1,11,0.1):
                ...
                print(data[str((i, j))])
        with open(f"../data/result{k}.pkl","wb") as f:
            pickle.dump(data,f)
